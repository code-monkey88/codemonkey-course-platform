...
### フェーズ1: 基盤構築（1-2日）
- [x] Next.js + Supabase プロジェクトセットアップ
- [x] Google OAuth設定（Google Cloud Console + Supabase）
- [x] データベーステーブル作成（SQL実行）
- [x] トリガー関数・RLSポリシー設定
- [x] 環境変数設定（`.env.local`）

### フェーズ2: 認証機能（1日）
- [x] Google認証実装（ログイン・ログアウト）
- [x] 認証コールバック処理
- [x] 認証ガードミドルウェア

### フェーズ3: コア機能（3-4日）
- [x] トップページ（講座一覧）
- [x] 講座詳細ページ（セクション・動画一覧）
- [x] 動画視聴ページ（YouTube埋め込み）
- [x] 進捗マーク機能（完了ボタン）
